...
def run_latency_bench(timeout_ms: int, num_batches: int = 50):
...
def run_e2e_bench(timeout_ms: int, num_simulations: int = 20, batch_size: int = 16):
...
def main():
    print("=" * 70)
    print("AlphaTafl Phase 5 IPC Benchmark")
    print("=" * 70)
    ...
    for timeout_ms in [50, 5, 0]:
        total, per_batch, per_state = run_latency_bench(timeout_ms, num_batches=50)
    ...
    for timeout_ms in [50, 5, 0]:
        game_time, num_turns = run_e2e_bench(timeout_ms, num_simulations=20, batch_size=16)
